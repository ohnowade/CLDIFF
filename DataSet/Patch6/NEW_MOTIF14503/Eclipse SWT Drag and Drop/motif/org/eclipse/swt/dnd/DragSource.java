...
public void anomaly1(DragSourceListener listener) {
	if (listener == null) DND.error (SWT.ERROR_NULL_ARGUMENT);
	DNDListener typedListener = new DNDListener (listener);
	typedListener.dndWidget = new DragSource(control, dragContext);
	addListener (DND.DragStart, typedListener);
	addListener (DND.DragSetData, typedListener);
	addListener (DND.DragEnd, typedListener);
}

public void anomaly2(DragSourceListener listener) {
	if (listener == null) DND.error (SWT.ERROR_NULL_ARGUMENT);
	DNDListener lis = new DNDListener (null);
	lis.dndWidget = this;
	addListener (DND.DragStart, lis);
	addListener (DND.DragSetData, lis);
	addListener (DND.DragEnd, lis);
}

public void anomaly3(DragSourceListener listener) {
	if (listener == null) DND.error (SWT.ERROR_NULL_ARGUMENT);
	DNDListener typedListener = new DNDListener (listener);
	typedListener = (DNDListener)listener;
	addListener (DND.DragStart, typedListener);
	addListener (DND.DragSetData, typedListener);
	addListener (DND.DragEnd, typedListener);
}

public void miss2(DragSourceListener listener) {
	if (listener == null) DND.error (SWT.ERROR_NULL_ARGUMENT);
	DNDListener lis = new DNDListener (listener);
	addListener (DND.DragStart, lis);
	addListener (DND.DragSetData, lis);
	addListener (DND.DragEnd, lis);
}
